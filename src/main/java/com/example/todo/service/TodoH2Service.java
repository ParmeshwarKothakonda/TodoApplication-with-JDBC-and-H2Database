...
public class TodoH2Service implements TodoRepository{
    ...
    public Todo addTodo(Todo todo){
        ...
        Todo savedTodo = db.queryForObject("select * from TODOLIST where todo = ? and priority = ? and status = ?", new TodoRowMapper(), todo.getTodo(),
        todo.getPriority(), todo.getStatus());

        return savedTodo; //Returning
    ...
    public Todo updateTodo(int id, Todo todo){
        ...
        try{
            Todo existingTodo = db.queryForObject("select * from TODOLIST where id = ?", new TodoRowMapper(), id);
            ...
            Todo updatedTodo = db.queryForObject("select * from TODOLIST where id =?", new TodoRowMapper(), id);

            return updatedTodo; //Returning
        }
        catch(Exception e){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
